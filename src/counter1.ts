...
export const initialVm: ICounterVm = { counter: 0, step: 2 };
...
const incr$ = incrSubj.pipe(
  ...
  map((delta) => (vm: ICounterVm) => ({ ...vm, counter: vm.counter + delta * vm.step }))
);
...
//   map((delta) => (vm: ICounterVm) => ({ ...vm, step: delta }))
  map((step) => (vm: ICounterVm) => ({ ...vm, step }))
);
...
export const vm$ = merge( incr$, decr$, step$).pipe(
    ...
    scan( (prevVm:ICounterVm, mutationFn:(vm:ICounterVm) => ICounterVm) => mutationFn(prevVm), initialVm)
)